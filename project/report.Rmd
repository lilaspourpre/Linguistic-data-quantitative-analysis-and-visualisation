...
str(embarr)
```
* Our dependent variable is MERV -- mean embarrassement recognition value: average score for all/French/Russian audios. Its type is numerical (ratio) and belongs to the interval from 0 to 5.For each audio we recieved the following maximum, minimum and average values:


![alt text](https://github.com/lilaspourpre/Linguistic-data-quantitative-analysis-and-visualisation/blob/master/project/img/audio_French.png)


![alt text](https://github.com/lilaspourpre/Linguistic-data-quantitative-analysis-and-visualisation/blob/master/project/img/audio_Russian.png)
...
We can see that difference of mean values (0,405) for Russian and French are slightly different. That is why we use t-test in order to discover whether this difference is staticstically significant.
...
We also ran a set of t-tests for discovering dependency of Russian, French and Russian+French mean embarrassement recognition values on each parameter. The following code is run to calculate the values of t-tests. 
```
#gender
t.test(embarr[embarr$Gender == "M",]$meanRussian, embarr[embarr$Gender == "F" ,]$meanRussian)
t.test(embarr[embarr$Gender == "M",]$meanFrench, embarr[embarr$Gender == "F" ,]$meanFrench)
t.test(embarr[embarr$Gender == "M",]$meanCommon, embarr[embarr$Gender == "F" ,]$meanCommon)

#education
t.test(embarr[embarr$Education == "Higher",]$meanRussian, embarr[embarr$Education == "Secondary",]$meanRussian)
t.test(embarr[embarr$Education == "Higher",]$meanFrench, embarr[embarr$Education == "Secondary" ,]$meanFrench)
t.test(embarr[embarr$Education == "Higher",]$meanCommon, embarr[embarr$Education == "Secondary" ,]$meanCommon)

#English
t.test(embarr[embarr$English == "1",]$meanRussian, embarr[embarr$English == "0" ,]$meanRussian)
t.test(embarr[embarr$English == "1",]$meanFrench, embarr[embarr$English == "0" ,]$meanFrench)
t.test(embarr[embarr$English == "1",]$meanCommon, embarr[embarr$English == "0" ,]$meanCommon)

#French
t.test(embarr[embarr$French == "1",]$meanRussian, embarr[embarr$French == "0" ,]$meanRussian)
t.test(embarr[embarr$French == "1",]$meanFrench, embarr[embarr$French == "0" ,]$meanFrench)
t.test(embarr[embarr$French == "1",]$meanCommon, embarr[embarr$French == "0" ,]$meanCommon)
```

The results might be seen in the table below:

| Parameters | t-value (RU) | p-value (RU)  | Statistically significant|
| --- | --- | --- | --- | 
| Gender | 0.220 | 0.982 || 
| Education | -1.517 | 0.130 || 
| English | -0.542 | 0.589 ||
| French | -0.823 | 0.411 ||

| Parameters | t-value (FR) | p-value (FR)  | Statistically significant|
| --- | --- | --- | --- | 
| Gender | -0.498 | 0.619  || 
| Education | -0.964 | 0.336 || 
| English | -2.87 | 0.004 | * |
| French |  1.470 | 0.140 ||

| Parameters | t-value (FR+RU) | p-value (FR+RU) | Statistically significant|
| --- | --- | --- | --- | 
| Gender | -0.317 | 0.752 || 
| Education | -1.559 | 0.12 || 
| English | -2.230 | 0.027 | * |
| French  | 1.470 | 0.14 ||

For multicategorical age parameter ANOVA is used.
```
#age
fit1 <- aov(meanRussian ~ Age, data = embarr)
summary(fit1)

fit2 <- aov(meanFrench ~ Age, data = embarr)
summary(fit2)

fit3 <- aov(meanCommon ~ Age, data = embarr)
summary(fit3)
```
