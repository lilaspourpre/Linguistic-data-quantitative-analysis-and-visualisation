...
It is rather obvious that our dataset is not as brilliant as it might seem at first glance. Binary type of language aquisition does not represent the actual language knowledge. The same problem might appear with age. That is why we apply data foratting and data reconsideration that is discussed in the next subsection

 ### Data formatting
 
Our dataset comprises 216 responces from questionnaire (described below). Our .csv file contains 216 rows and 25 columns.

* Our dependent variable is theprecision value 

To add, in order to check this hypothesis we have to evaluate the results for each audio so we standardize the values by dividing each by 5 as we believe that MERV is equal to 5.  
-- mean embarrassement recognition value.
